...
export default function Index({
    ...
    const handleDeleteInvoiceGeneral = () => {
        destroy("invoice_generals/" + selectedInvoiceGeneral.uuid, {
            onSuccess: () => {
                getInvoiceGenerals();
                showSuccess("Hapus");
            },
    ...
    const columns = [
...

        {
            ...
            body: (rowData) => {
                return (
                    <div className="flex w-full h-full items-center justify-center">
                        <a
                            href={rowData.invoice_general_doc}
                            download={`Invoice_General_${rowData.institution_name}`}
                            class="font-bold  w-full h-full text-center rounded-full "
                        >
                            <i
                                className="pi pi-file-pdf"
        ...
        },
        {
            ...
            body: (rowData) => {
                return rowData.total.toLocaleString("id");
            },
        },
        {
            field: "total_all_ppn",
            ...
            body: (rowData) => {
                return rowData.total_all_ppn.toLocaleString("id");
            },
        },
        {
            field: "paid_off",
    ...
    const objectKeyToIndo = (key) => {
        ...
        if (firstKey == "institution_name") {
            keyIndo = "Lembaga";
        } else if (firstKey == "institution_npwp") {
            keyIndo = "NPWP";
                ...
                >
                    ...
                    <TabPanel header="Log">
                        {activeIndexTab == 1 && (
                            <LogComponent
                                auth={auth}
                                fetchUrl={"/api/invoice_generals/logs"}
                                filterUrl={"/invoice_generals/logs/filter"}
                                deleteUrl={"/invoice_generals/logs"}
                    ...
                    <TabPanel header="Arsip">
                        {activeIndexTab == 2 && (
                            <ArsipComponent
                                auth={auth}
                                users={users}
                                fetchUrl={"/api/invoice_generals/arsip"}
                                forceDeleteUrl={"/invoice_generals/{id}/force"}
                                restoreUrl={"/invoice_generals/{id}/restore"}
                                filterUrl={"/invoice_generals/arsip/filter"}
                                columns={columns}
                                showSuccess={showSuccess}
                                showError={showError}
